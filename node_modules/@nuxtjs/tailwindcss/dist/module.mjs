import { relative, join, dirname, resolve, isAbsolute } from 'pathe';
...
const createInternalContext = async (moduleOptions, nuxt = useNuxt()) => {
  ...
  const resolveConfigs = (configs, nuxt2 = useNuxt()) => (Array.isArray(configs) ? configs : [configs]).filter(Boolean).map(async (config, idx, arr) => {
